                ...
                <Grid item xs={2} style={{position: 'relative'}}>
                    {
                        isContact != null ?
                            ...
                            <Avatar className={isContact.favorite ? classes.avatarFav : classes.avatar} style={{ background: isContact.color }}>{isContact.name.charAt(0)}</Avatar>) :
                            <Avatar className={classes.avatar}>#</Avatar>
                    }
                    { props.unread > 0 ? <div className={classes.unread}>{props.unread}</div> : null }
                </Grid>
                <Grid item xs={10}>
                    {
                        isContact != null ?
                        <div className={classes.number}>{isContact.name}</div> :
                        <div className={classes.number}>{props.message.number}</div>
                    }
                    <div className={classes.message}>{props.message.message}</div>
                    <div className={classes.time}><Moment fromNow>{+props.message.time}</Moment></div>
                </Grid>
            </Grid>
        </Paper>
    )  
