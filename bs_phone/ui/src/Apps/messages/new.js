...
export default connect(null, { showAlert })((props) => {
	...
	useEffect(() => {
		setFilteredContacts(contacts.filter(c => c.name.toUpperCase().includes(searchVal.toUpperCase())));
	}, [searchVal]);
                    ...
                    contacts.length > 0 ? (
                        <div style={{ height: '100%' }}>
                            ...
                            <div className={classes.contactsBody}>
                                {
                                    filteredContacts.filter(c => c.favorite).sort((a, b) => {
                                        if (a.name.toLowerCase() > b.name.toLowerCase()) return 1;
                                        else if (b.name.toLowerCase() > a.name.toLowerCase()) return -1;
                                        else return 0;
                                    }).map((contact) => {
                                        return(
                                            <Grid container key={contact._id} className={classes.contactWrapper} onClick={() => onContactClick(contact)}>
                                                <Grid item xs={2}>
                                                    {
                                                        contact.avatar != null && contact.avatar !== '' ?
                                                        <Avatar className={classes.avatarFav} src={contact.avatar} alt={contact.name.charAt(0)} /> :
                                                        <Avatar className={classes.avatarFav} style={{ background: contact.color }}>{contact.name.charAt(0)}</Avatar>
                                                    }
                                                </Grid>
                                                <Grid item xs={10}>
                                        ...
                                        return(
                                            <Grid container key={contact._id} className={classes.contactWrapper} onClick={() => onContactClick(contact)}>
                                                <Grid item xs={2}>
                                                    {
                                                        contact.avatar != null && contact.avatar !== '' ?
                                                        <Avatar className={classes.avatar} src={contact.avatar} alt={contact.name.charAt(0)} /> :
                                                        <Avatar className={classes.avatar} style={{ background: contact.color }}>{contact.name.charAt(0)}</Avatar>
                                                    }
                                                </Grid>
                                                <Grid item xs={10}>
                                                    <div className={classes.contactData}>
