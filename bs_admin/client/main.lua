...
AddEventHandler('Admin:Revive:Client:WithinRange', function(coords)
    ...
    if IsPedFatallyInjured(myPed) then
        ...
        local distance = #
            (vector3(coords.x, coords.y, coords.z) - vector3(playerCoords.x, playerCoords.y, playerCoords.z))
            ...
            Callbacks:ServerCallback('Admin:receiveRecentDisconnects', {}, function(recentDisconnectsData)
                Callbacks:ServerCallback('Admin:receiveActivePlayers', {}, function(activePlayersData)
                    Callbacks:ServerCallback('Admin:receiveSpawnLocations', {}, function(spawnLocations)
                        ...
                        local balanceManagement = Menu:Create('balanceManagement', 'Balance Management')
                        ...
                        adminUtilities.Add:Button((noClip and "Disable NoClip" or "Enable NoClip"),
                            { disabled = false, success = noClip }, function(data)
                            noClip = not noClip
                            TriggerEvent('Admin:noclip')
                            Notification:SendAlert('NoClip has been: ' .. (noClip and "Enabled" or "Disabled"), 2500)
                            adminUtilities.Update:Item(data.id, (noClip and "Disable NoClip" or "Enable NoClip"),
                                { success = noClip })
                            root.Close()
                        end)

                        adminUtilities.Add:Button((showCoords and "Disable Coords" or "Enable Coords"),
                            { disabled = false, success = showCoords }, function(data)
                            showCoords = not showCoords
                            Notification:SendAlert('Coordinates have been ' .. (showCoords and "Enabled" or "Disabled"),
                                3500)
                        ...
                        if recentDisconnectsData ~= nil then
                            ...
                            if not found then
                                recentDisconnects.Add:Button("No Recent Disconnects", { disabled = true },
                                    function(data)
                        ...
                        if activePlayersData ~= nil then
                            ...
                            for t, q in pairs(activePlayersData) do
                                ...
                                q.subMenu = Menu:Create('player' .. q.source .. 'subMenu',
                                    'More Info - [' .. q.source .. '] ' .. q.name)
                                q.banMenu = Menu:Create('player' .. q.source .. 'banMenu',
                                    'Ban Menu - [' .. q.source .. '] ' .. q.name)
                                q.kickMenu = Menu:Create('player' .. q.source .. 'kickMenu',
                                    'Kick Menu - [' .. q.source .. '] ' .. q.name)
                                ...
                                q.menu.Add:Button("Bring Player", { disabled = false }, function(data)
                                    ...
                                    SetEntityCoords(GetPlayerPed(GetPlayerFromServerId(q.source)), myCoords.x, myCoords.y
                                        , myCoords.z)
                                    root.Close()
                                end)
                                ...
                                q.kickMenu.Add:Button("Kick Player", { disabled = false, success = true },
                                    function(data)
                                        if kickReason then
                                ...
                                q.banMenu.Add:Button("Ban Player", { disabled = false, success = true },
                                    function(data)
                                        if banReason and banTime and banTime > -2 then
                                            ...
                                            Notification:SendAlert(q.name ..
                                                ' has been banned from the server ' ..
                                                (banTime == -1 and "permanently" or "for " .. lengthName), 3500)
                                            root.Close()
                                        else
                                            Notification:SendError('You need to complete all the required information',
                                                3500)
                                        end
                                    end)
                        ...
                        teleportTo.Add:Button("Teleport to Waypoint", { disabled = false, success = true },
                            function(data)
                                local success = false
                        ...
                        vehicleManagement.Add:Button("Spawn Vehicle", { disabled = false, success = true },
                            function(data)
                                if spawnVeh then
                        ...
                        if spawnLocations ~= nil then
                            for k, v in pairs(spawnLocations) do
                                spawnLoc.Add:Button(v.Name, { disabled = false }, function(data)
                                    SetEntityCoords(PlayerPedId(), tonumber(v.Coords.x), tonumber(v.Coords.y),
                                        tonumber(v.Coords.z))
                                    root.Close()
                                end)
                            end
                        ...
                        adminUtilities.Add:SubMenu('Give Cash', balanceManagement)

                        balanceManagement.Add:Input('Cash', {
                            disabled = false,
                            max = 30,
                            current = "0",
                        }, function(data)
                            spawnCash = tonumber(data.data.value)
                        end)

                        balanceManagement.Add:Button("Give", { disabled = false, success = true },
                            function(data)
                                if spawnCash then
                                    TriggerServerEvent('Wallet:Server:GiveCash', spawnCash)
                                else
                                    Notification:SendError("You have not specified a amount of cash to give.")
                                end
                            end)

                        balanceManagement.Add:Button("Remove", { disabled = false }, function(data)
                            if spawnCash then
                                TriggerServerEvent('Wallet:Server:RemoveCash', spawnCash)
                            else
                                Notification:SendError("You have not specified a amount of cash to remove.")
                            end
                        end)

                        balanceManagement.Add:SubMenuBack('Back')
