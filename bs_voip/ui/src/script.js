...
function init() {
	...
	websocket.onmessage = (evt) => {
		...
		if (evt.data == 'startedtalking') {
			$.post('http://bs_voip/setPlayerTalking', JSON.stringify({state: 1}));
		}
		if (evt.data == 'stoppedtalking') {
			$.post('http://bs_voip/setPlayerTalking', JSON.stringify({state: 0}));
		}
	};
...
function updateScriptData(key, data) {
	...
	$.post(`http://bs_voip/updatePluginData`, JSON.stringify({
		payload: {
			key,
			data,
		}
