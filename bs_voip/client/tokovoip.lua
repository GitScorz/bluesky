...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if previousLevel ~= self.mode then
		previousLevel = self.mode
		SendNUIMessage({
			type = 'SET_RANGE',
			data = { 
				range = self.mode,
	...
	if (self.plugin_data.radioChannel ~= -1 and self.myChannels[self.plugin_data.radioChannel]) then
		if self.plugin_data.radioChannel > 999 then
...
function TokoVoip.updatePlugin(self, event, payload)
	exports.bs_voip:doSendNuiMessage(event, payload);
end
...
function TokoVoip.initialize(self)
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1 and self.config.radioEnabled and not IsPedFatallyInjured(PlayerPedId())) then -- Talk on radio
