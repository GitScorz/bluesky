...
function removePlayerFromCall(source, callChannel)
    Logger:Info(nil, 'Voip', ("Removed %s from call %s"):format(source, callChannel))
...
function addPlayerToCall(source, callChannel)
    Logger:Info(nil, 'Voip', ('Added %s to call %s'):format(source, callChannel))
...
function setPlayerCall(source, _callChannel)
    ...
    if not callChannel then
		...
		if isResource then
			Logger:Error(nil, 'Voip', ("'callChannel' expected 'number', got: %s"):format(type(_callChannel)))
		else
			return Logger:Warn(nil, 'Voip', ("%s sent a invalid call, 'callChannel' expected 'number', got: %s"):format(source,type(_callChannel)))
		end
	end
	if isResource then
...
function setTalkingOnCall(talking)
    ...
    if callTbl then
        Logger:Info(nil, 'Voip', ('%s as %s talking on call %s'):format(source, talking and 'started' or 'stopped', plyVoice.call))
        for player, _ in pairs(callTbl) do
            if player ~= source then
                Logger:Info(nil, 'Voip', ('Sending event to %s to tell them that %s is talking'):format(player, source))
                TriggerClientEvent('pma-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
        Logger:Info(nil, 'Voip', ('%s tried to talk in call %s, but it doesnt exist.'):format(source, plyVoice.call))
    end
end
