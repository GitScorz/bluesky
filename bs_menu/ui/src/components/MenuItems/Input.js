...
export default props => {
  ...
  const [value, setValue] = useState(props.data.options.current == null ? '' : props.data.options.current);
  ...
  return (
    <div className={cssClass} style={style}>
      <TextField
        ...
        type='text'
        multiline
        inputProps={{ maxLength: props.data.options.max != null ? props.data.options.max : 1024 }}
      />
    </div>
  );
};
