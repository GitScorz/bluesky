...
exports('RegisterComponent', function(component, data)
    ...
    if COMPONENTS[component] ~= nil then
        if COMPONENTS[component]._protected then
            COMPONENTS.Logger:Warn('Proxy', 'Attempt To Override Protected Component: ' .. tostring(component), { console = true })
            return
        else
            if COMPONENTS[component]._required and #COMPONENTS[component]._required > 0 then
                for k, v in ipairs(COMPONENTS[component]._required) do
                    if data[v] == nil then
                        COMPONENTS.Logger:Warn('Proxy', 'Attempt To Extend Component While Missing Required Attribute: ' .. tostring(v), { console = true })
                        return
                    end
                end
            end
        ...
        _overriding = true
        COMPONENTS.Logger:Trace('Proxy', 'Overriding Existing Component: ' .. tostring(component), { console = true })
    end
...
exports('FetchComponent', function(component)
    if not COMPONENTS[component] then
        COMPONENTS.Logger:Warn('Proxy', 'Attempt To Fetch Non-Existent Component: ' .. tostring(component), { console = true })
        return nil
    end
...
exports('ExtendComponent', function(component, data)
    if COMPONENTS[component] ~= nil then
        if COMPONENTS[component]._protected == nil then
            ...
            COMPONENTS.Logger:Warn('Proxy', 'Attempt To Extend Protected Component: ' .. tostring(component), { console = true })
        end
    else
        COMPONENTS.Logger:Warn('Proxy', 'Attempt To Extend Non-Existent Component: ' .. tostring(component), { console = true })
    end
end)
