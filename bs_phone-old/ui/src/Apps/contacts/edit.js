...
export default connect(null, { showAlert, updateContact })(props => {
	...
	const onSubmit = e => {
        ...
        if (contacts.filter(c => c.number === contact.number && c._id != contact._id).length > 1 && contacts.filter(c => c._id === contact._id && c.number !== contact.number).length > 0) {
            props.showAlert('Contact Already Exists For This Number');
        } else {
            props.updateContact(contact._id, contact);
            props.showAlert(`${contact.name} Updated`);
            navigate(-1);
        }
	...
	return (
		<div className={classes.wrapper}>
            <Paper className={classes.contactHeader}>
                ...
                <div className={classes.contactName} style={{color: contact.color}}>{contact.name}</div>
                <div className={classes.contactNumber}>{contact.number}</div>
                <Grid container className={classes.contactButtons}>
                    <Grid item xs={6} onClick={callContact} className={classes.contactButton}>
                        <FontAwesomeIcon icon="phone" />
                    </Grid>
                    <Grid item xs={6} onClick={textContact} className={classes.contactButton}>
                        <FontAwesomeIcon icon="sms" />
                    </Grid>
                </Grid>
            </Paper>
            <Paper className={classes.actions}>
                <div className={classes.action}>View Call History</div>
            </Paper>
			<Paper className={classes.editBody}>
                <div style={{ textAlign: 'center' }}><h1>Edit Contact Details</h1></div>
            ...
            <ButtonGroup variant="text" color="primary" className={classes.buttons}>
                <Button className={classes.button} onClick={() => navigate(-1)}>Cancel</Button>
                <Button className={classes.buttonNegative} type='submit' form='contact-form'>Delete</Button>
