...
---------------------------------------------------------------------
RegisterNetEvent("VehControl:TogIndicState_c")
AddEventHandler("VehControl:TogIndicState_c", function(sender, newstate)
...
---------------------------------------------------------------------
RegisterNetEvent("VehControl:TogDfltSrnMuted_c")
AddEventHandler("VehControl:TogDfltSrnMuted_c", function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent("VehControl:SetLxSirenState_c")
AddEventHandler("VehControl:SetLxSirenState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent("VehControl:TogPwrcallState_c")
AddEventHandler("VehControl:TogPwrcallState_c", function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent("VehControl:SetAirManuState_c")
AddEventHandler("VehControl:SetAirManuState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if count_bcast_timer > delay_bcast_timer then
							...
							--- IS EMERG VEHICLE ---
							if GetVehicleClass(veh) == 18 then
								TriggerServerEvent("VehControl:TogDfltSrnMuted_s", dsrn_mute)
								TriggerServerEvent("VehControl:SetLxSirenState_s", state_lxsiren[veh])
								TriggerServerEvent("VehControl:TogPwrcallState_s", state_pwrcall[veh])
								TriggerServerEvent("VehControl:SetAirManuState_s", state_airmanu[veh])
							end
							--- IS ANY OTHER VEHICLE ---
							TriggerServerEvent("VehControl:TogIndicState_s", state_indic[veh])
						else
