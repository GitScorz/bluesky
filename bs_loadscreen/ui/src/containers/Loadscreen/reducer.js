...
const appReducer = (state = initialState, action) => {
	switch (action.type) {
		...
		case 'endDataFileEntries':
            if (action.payload.type == null || state.stages[action.payload.type] == null) return state;
            count++;
            let t = { ...state };

            if (action.payload.type == 'INIT_SESSION' && t.stages.INIT_BEFORE_MAP_LOADED < 1) {
            ...
            if (state.currentStage != null && action.payload.type !== state.currentStage && t.completed[state.currentStage] == null) {
