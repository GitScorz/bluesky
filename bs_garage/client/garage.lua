...
function RegisterBlips()
  for k, v in ipairs(Config.Garages) do
    Blips:Add(v.Name, v.Name, v.Manage, 357, 74, 0.8, 10)
  end
end
...
function RegisterMarkers()
  for k, v in ipairs(Config.Garages) do
    ...
    Markers.Markers:Add(v.Name, v.Name .. '_manage', v.Manage, 2, vector3(1, 1, 1), { r = 255, b = 0, g = 0 },
      function()
        return true
      end, 'Parking', 1, function()
      OpenMenu(v.Name, v.Spawn)
    end)
    Markers.Markers:Add(v.Name, v.Name .. '_store', v.Store, 1, vector3(5, 5, 3), { r = 255, b = 255, g = 0 },
      function()
        return true
...
function OpenMenu(location, spawnCoords)
  ...
  Callbacks:ServerCallback('Garage:GetAllVehicles', {}, function(vehicles)
    for _, v in pairs(vehicles) do
      local menuName = v.Plate ..
          ' - ' .. GetDisplayNameFromVehicleModel(v.Properties.model) .. ' - ' .. v.Location
          ...
          Callbacks:ServerCallback('Garage:ReleaseVehicle', {
            ...
            if result then
              Callbacks:ServerCallback('Garage:GetVehicle', {
                ...
                Game.Vehicles:Spawn(spawnCoords, vehicle.Properties.model, 0.0,
